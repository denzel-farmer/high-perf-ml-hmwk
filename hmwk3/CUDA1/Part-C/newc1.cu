...
__global__ void Convolution(Image in_image, FilterSet filters, Image out_image){
    ...
    int k_init = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    // if (x < in_image.width && y < in_image.height) {

    for (int k = k_init; k < out_image.depth; k += gridDim.x) {
    ...
    for (int c = 0; c < in_image.depth; c++) {
        for (int j = 0; j < filters.height; j++) {
            for (int i = 0; i < filters.width; i++) {
                ...
                // I_0[c, x + i, y + j]
                int filter_i = filters.width - 1 - i;
                int filter_j = filters.height - 1 - j;
                double filter_value = filters.elements[(k * filters.height * filters.width * filters.depth) + (c * filters.height * filters.width) \
                + (filter_j * filters.width) + filter_i];


                int image_i = x + i;
                int image_j = y + j;
                double image_value = in_image.elements[(c * in_image.height * in_image.width) + (image_j * in_image.width) + image_i];

                output_value += filter_value * image_value;


                
                //output_value += filter_value * GetImageElement(in_image, c, x + i, y + j);
            }
        }
    }

    out_image.elements[(k * out_image.height * out_image.width) + (y * out_image.width) + x] = output_value;
...
//    dim3 dimBlock(block_dim_x, block_dim_y, block_dim_z);
    ...
    // dim3 dimGrid(num_blocks_z, num_blocks_x, num_blocks_y);
    dim3 dimGrid(2, 32, 32);
        ...
        for (int y = 0; y < H; y++) {
            for (int x = 0; x < W; x++) {
               ...
               //double value = 1;
                ...
                if ((x < padding) || (y < padding) || (y > (H - padding - 1))  || (x > (W - padding - 1)))
                    value = 0.0;
...
int main(int argc, char* argv[]) {
    ...
    // Image temp = GenerateInputImage(3, 4, 4, 1);
    // PrintImage(temp, 1024);
